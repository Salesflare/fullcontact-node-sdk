...
class API {
  constructor(c = {}) {
    ...
    this._fullResponse = c.returnFullResponse || false;
  }
  ...
  /**
   * Gets option to return a full response instead of just the body from the config
   * 
   * @readonly
   * 
   * @memberOf API
   */
  get returnFullResponse(){
  ...
  async request(opts = {}) {
    ...
    return request({
      ...
      json: opts.json,
      resolveWithFullResponse: this._fullResponse
    });
  }
}
